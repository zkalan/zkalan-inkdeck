...
当前版本 0.5.0。63 项模型检查和 118 项应用检查全部通过；验证使用合成触控/压力以及真实桌面窗口，实物书写手感和会议软件的共享效果仍需试用确认。
...
| `K` /「调色」 | 打开 macOS 调色盘 |
| `M` / 输入方式下拉框 | 切换按住空格画图 / 轻触连续书写，自动记住选择 |
| `Esc` | 结束书写并恢复普通光标 |
...
- 默认按住空格画图适合示意图；「轻触连续书写」是可选模式。两者都使用同一套绝对坐标。连续书写模式中按住空格预览，松开恢复书写。
