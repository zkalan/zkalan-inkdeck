...
[下载应用](https://github.com/zkalan/zkalan-inkdeck/releases/tag/v0.5.0) · [备用下载与校验文件](downloads) · [验证记录](artifacts/v0.5/VALIDATION.md) · [版本说明](CHANGELOG.md) · [代码来源与依赖](PROVENANCE.md)

当前版本 0.5.0。63 项模型检查和 130 项应用检查全部通过；验证使用合成触控/压力以及真实桌面窗口，实物书写手感和会议软件的共享效果仍需试用确认。

![调色与局部擦除](artifacts/v0.5/tools-demo.png)

上图由合成触控输入生成，展示自定义颜色和局部擦除，不包含用户草稿。

## 打开

从 GitHub Release 或本仓库的 `downloads` 下载 `ZkalanInkDeck-0.5.0-macOS-arm64.zip`，解压后将应用拖到「应用程序」或 `~/Applications`，双击打开。窗口副标题为 `0.5 · 示意图与橡皮擦`，原型旧称「触控板手写 2」，新版会读取已有草稿。
...
4. 点击工具条的 **继续标记** 恢复书写。暂停时可调整最近颜色、普通笔 / 毛笔与笔宽；也可直接按 K 打开调色盘，也可以拖动工具条。
...
- 按 **E** 切换橡皮擦，再按 E 返回画笔。橡皮擦模式下，手指接触并移动就会擦除经过的局部区域；抬手结束这次擦除。圆圈显示擦除范围，旁边滑块调节大小，画板缩放时保持相同的屏幕直径。
...
11. 按 E 擦掉横线的中段，确认两侧笔迹保留；Z 恢复，Y 重做，放大后检查擦除圆圈大小不变。
12. 按 M 切换连续书写，确认轻触就写、按住空格预览；再按 M 切换按压落笔，轻触不留痕、按下后开始、释放压力停止。再按 M 返回默认。
13. 从菜单栏进入桌面标记，圈出下面的软件元素；Enter 暂停后点击/滚动该软件，再恢复标记。检查退出后光标正常、主画板内容未变。
...
`artifacts/v0.5/` 存放结果 JSON、画板与桌面工具条、帮助、透明层、合成演示及导出图片。低压力示例从上到下为原始压力 0%、2%、10%，每条线交替慢速和快速；这些是合成输入。自动测试不读取或修改用户草稿，也不截取桌面内容，不能替代真实硬件触控、压力采样和手感测试。
