...
set -euo pipefail
project_dir="${0:A:h:h}"
app_dir="${1:-$project_dir/dist/Zkalan InkDeck.app}"
...
# Register the rebuilt bundle before asking LaunchServices for foreground activation.
...
if not result.exists() or result.stat().st_mtime_ns < marker.stat().st_mtime_ns:
    raise SystemExit('Smoke test did not produce a fresh result; check the desktop session and retry.')
data = json.loads(result.read_text())
if data.get('testSetupError'):
    raise SystemExit('Setup failed: ' + data['testSetupError'])
checks = {key: value for key, value in data.items() if type(value) is bool}
